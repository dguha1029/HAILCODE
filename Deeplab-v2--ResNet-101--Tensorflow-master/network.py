...
To use the pre-trained models, the name of each layer is the same as that in .ckpt file.
"""
...
class Deeplab_v2(object):
	...
	def _conv2d(self, x, kernel_size, num_o, stride, name, biased=False):
		...
		with tf.compat.v1.variable_scope(name) as scope:
			w = tf.compat.v1.get_variable('weights', shape=[int(kernel_size), int(kernel_size), int(num_x), int(num_o)])
			s = [1, stride, stride, 1]
	...
	def _batch_norm(self, x, name, is_training, activation_fn, trainable=False):
		...
		'''with tf.variable_scope(name) as scope:
			...
			return o'''
		#with tf.compat.v1.variable_scope(name) as scope:
		o = tf.keras.layers.BatchNormalization(
				name=name,
				scale=True,
				center=True,
				trainable=trainable,
