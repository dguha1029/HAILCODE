...
def image_scaling(img, label):
    ...
    scale = tf.compat.v1.random_uniform([1], minval=0.5, maxval=1.5, dtype=tf.float32, seed=None)
    h_new = tf.compat.v1.to_int32(tf.multiply(tf.compat.v1.to_float(tf.shape(img)[0]), scale))
    w_new = tf.compat.v1.to_int32(tf.multiply(tf.compat.v1.to_float(tf.shape(img)[1]), scale))
    new_shape = tf.compat.v1.squeeze(tf.stack([h_new, w_new]), squeeze_dims=[1])
    img = tf.compat.v1.image.resize_images(img, new_shape)
    label = tf.compat.v1.image.resize_nearest_neighbor(tf.expand_dims(label, 0), new_shape)
    label = tf.compat.v1.squeeze(label, squeeze_dims=[0])
...
def image_mirroring(img, label):
    ...
    distort_left_right_random = tf.compat.v1.random_uniform([1], 0, 1.0, dtype=tf.float32)[0]
...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255):
    ...
    combined_crop = tf.compat.v1.random_crop(combined_pad, [crop_h, crop_w, 4])
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror, ignore_label, img_mean): # optional pre-processing arguments
    ...
    img_contents = tf.compat.v1.read_file(input_queue[0])
    label_contents = tf.compat.v1.read_file(input_queue[1])
