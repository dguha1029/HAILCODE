...
class Model(object):
	...
	def train_setup(self):
		...
		raw_output_up = tf.compat.v1.image.resize_bilinear(raw_output, input_size)
		raw_output_up = tf.argmax(raw_output_up, axis=3)
		self.pred = tf.compat.v1.expand_dims(raw_output_up, dim=3)
		# Image summary.
		images_summary = tf.py_function(inv_preprocess, [self.image_batch, 2, IMG_MEAN], tf.uint8)
		labels_summary = tf.py_function(decode_labels, [self.label_batch, 2, self.conf.num_classes], tf.uint8)
		preds_summary = tf.py_function(decode_labels, [self.pred, 2, self.conf.num_classes], tf.uint8)
	...
	def predict_setup(self):
		...
		# Predictions.
		raw_output = net.outputs
		raw_output = tf.compat.v1.image.resize_bilinear(raw_output, tf.shape(image_batch)[1:3,])
		raw_output = tf.argmax(raw_output, axis=3)
